...
const createIntern = async function (req, res) {
    try {
        let Data = req.body
        const { name, email, mobile, collegName} = Data
        //--------------------------->> Check Validation <<---------------------------->>>
        ...
        if (!collegName) return res.status(400).send({ status: false, msg: "please provide the collageName in request body" })
        ...
        if (!isValid(collegName)) return res.status(400).send({ status: false, msg: "please provide the valide coolageId" })
        ...
        let isvalidCollage = await collageModel.findOne({name:collegName})
